/**
 * Capture /service/curate-event for visual comparison with the design mock.
 *
 * Usage:
 *   1. Start the app: `npm run dev`
 *   2. `npm run visual:curate-event`
 *   3. Compare `visual-regression/output/curate-event-capture.png` to
 *      `visual-regression/reference/curate-event-design.png`
 *
 * Optional: `CURATE_EVENT_VISUAL_DIFF=1 npm run visual:curate-event` writes
 * `curate-event-diff.png` when reference and capture dimensions match (pixelmatch).
 *
...
const refPath = path.join(
	...
	"visual-regression",
	"reference",
	"curate-event-design.png",
);
const outPath = path.join(outDir, "curate-event-capture.png");
const diffPath = path.join(outDir, "curate-event-diff.png");
...
await page.goto(`${baseUrl}/service/curate-event`, {
	waitUntil: "networkidle0",
	timeout: 60_000,
});
await page.waitForSelector('[data-testid="curate-event-screen"]', { timeout: 30_000 });
await page.evaluate(() => {
...
if (process.env.CURATE_EVENT_VISUAL_DIFF === "1" && fs.existsSync(refPath)) {
