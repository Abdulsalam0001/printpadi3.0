/**
 * Capture /service/curate-event?visualStep=style for visual comparison.
 *
 * Usage:
 *   1. Start the app: `npm run dev`
 *   2. `npm run visual:curate-event-style`
 *   3. Compare `visual-regression/output/curate-event-style-capture.png` to
 *      `visual-regression/reference/curate-event-style-design.png`
 *
 * Optional: `CURATE_EVENT_STYLE_VISUAL_DIFF=1 npm run visual:curate-event-style`
 */
...
const refPath = path.join(
	...
	"visual-regression",
	"reference",
	"curate-event-style-design.png",
);
const outPath = path.join(outDir, "curate-event-style-capture.png");
const diffPath = path.join(outDir, "curate-event-style-diff.png");
...
await page.goto(`${baseUrl}/service/curate-event?visualStep=style`, {
	waitUntil: "load",
	timeout: 60_000,
});
await page.waitForSelector('[data-testid="curate-event-style-step"]', {
	timeout: 30_000,
});
await page.evaluate(() => {
...
if (process.env.CURATE_EVENT_STYLE_VISUAL_DIFF === "1" && fs.existsSync(refPath)) {
