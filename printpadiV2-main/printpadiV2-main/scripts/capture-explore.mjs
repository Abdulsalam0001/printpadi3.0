/**
 * Capture /explore for visual comparison with the design mock.
 *
 * Usage:
 *   1. Start the app: `npm run dev` (or `npm run build && npm run start`)
 *   2. `npm run visual:explore`
 *   3. Compare `visual-regression/output/explore-capture.png` to
 *      `visual-regression/reference/explore-design.png`
 *
 * Optional: `EXPLORE_VISUAL_DIFF=1 npm run visual:explore` writes `explore-diff.png`
...
const refPath = path.join(root, "visual-regression", "reference", "explore-design.png");
const outPath = path.join(outDir, "explore-capture.png");
const diffPath = path.join(outDir, "explore-diff.png");
...
await page.waitForSelector('[data-testid="explore-screen"]', { timeout: 30_000 });
...
if (process.env.EXPLORE_VISUAL_DIFF === "1" && fs.existsSync(refPath)) {
