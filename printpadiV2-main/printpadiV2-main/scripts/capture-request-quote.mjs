/**
 * Capture /service/request-quote for visual comparison with the design mock.
 *
 * Usage:
 *   1. Start the app: `npm run dev`
 *   2. `npm run visual:request-quote`
 *   3. Compare `visual-regression/output/request-quote-capture.png` to
 *      `visual-regression/reference/request-quote-design.png`
 *
 * Optional: `REQUEST_QUOTE_VISUAL_DIFF=1 npm run visual:request-quote` writes
 * `request-quote-diff.png` when reference and capture dimensions match (pixelmatch).
 *
...
const refPath = path.join(
	...
	"visual-regression",
	"reference",
	"request-quote-design.png",
);
const outPath = path.join(outDir, "request-quote-capture.png");
const diffPath = path.join(outDir, "request-quote-diff.png");
...
await page.goto(`${baseUrl}/service/request-quote`, {
	waitUntil: "networkidle0",
	timeout: 60_000,
});
await page.waitForSelector('[data-testid="request-quote-screen"]', { timeout: 30_000 });
await page.evaluate(() => {
...
if (process.env.REQUEST_QUOTE_VISUAL_DIFF === "1" && fs.existsSync(refPath)) {
	...
	if (img1.width !== img2.width) {
		...
		const scaled = new PNG({ width: img2.width, height: Math.round(img1.height * scale) });
