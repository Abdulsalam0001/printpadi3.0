/**
 * Capture /service/curate-event?visualStep=details for visual comparison.
 *
 * Usage:
 *   1. Start the app: `npm run dev`
 *   2. `npm run visual:curate-event-details`
 *   3. Compare `visual-regression/output/curate-event-details-capture.png` to
 *      `visual-regression/reference/curate-event-details-design.png`
 *
 * Optional: `CURATE_EVENT_DETAILS_VISUAL_DIFF=1 npm run visual:curate-event-details`
 */
...
const refPath = path.join(
	...
	"visual-regression",
	"reference",
	"curate-event-details-design.png",
);
const outPath = path.join(outDir, "curate-event-details-capture.png");
const diffPath = path.join(outDir, "curate-event-details-diff.png");
...
await page.goto(`${baseUrl}/service/curate-event?visualStep=details`, {
	waitUntil: "load",
	timeout: 60_000,
});
await page.waitForSelector('[data-testid="curate-event-details-step"]', {
	timeout: 30_000,
});
await page.evaluate(() => {
...
if (
	process.env.CURATE_EVENT_DETAILS_VISUAL_DIFF === "1" &&
